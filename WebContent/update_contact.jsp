...
<s:set var="contact" value="contact" />
  <jsp:useBean id="contact" type="com.contacts.beans.Contact" />
  <s:set var="cphone" value="cphone" />
  <jsp:useBean id="cphone" type="com.contacts.beans.ContactPhone" />
...
<button onclick="updatephone(this);return false;" name="update">update</button>
  ...
  <%
  for(int cnt=0;cnt<cphone.getPhoneid().size();cnt++)
  {
  %>
  <tr>
  <td><input type="radio" name="phone_id" value=<%= cphone.getPhoneid().get(cnt) %>></td>
  <td><input type="text" name="phone_number" value=<%= cphone.getPhonenumber().get(cnt) %>></td>
  </tr>
  <%
  }
  %>
